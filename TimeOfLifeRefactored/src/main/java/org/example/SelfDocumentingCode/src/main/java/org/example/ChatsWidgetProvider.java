package org.example.SelfDocumentingCode.src.main.java.org.example;
...
/*
РЕФЛЕКСИЯ:
Добавлять предваряющий комментарий - пока один из самых простых и очень нужных советов по работе с чем-то хоть сколько-то крупным.
Главное - возмжоность прямо сформулировать смысл написанного ниже (может быть полезно даже в случаях,
когда код пока сам по себе не работает так, как задумано) и добавить тут же список (субъективный) принципиальных зависимостей
- где описываемый класс нужен впоследствии, и от какого кода зависит сам.

Во время работы над проектом способ позволяет не терять нить (мышление на третьем уровне),
а после -  может экономить десятки часов, и себе, и другим, сохраняя нужный контекст.

Именно на материале этого примера - в исходном коде Телеграма 1496 отдельных классов и хотя бы несколько сократить эту сложность,
имея в начале каждого класса прямое указание, зачем его придумывали и добавили в проект - уже значительно упростило бы чтение кода.
 */


/*
Класс ChatsWidgetProvider управляет виджетом, отображающим список чатов на основном экране телефона.
Он обновляет и меняет размер виджета, использует класс SharedPreferences для сохранения настроек виджета,
и удаленную службу (ChatsWidgetService) для заполнения виджета данными чата.

Используется в классе EditWidgetActivity (субмодуль /ui).
