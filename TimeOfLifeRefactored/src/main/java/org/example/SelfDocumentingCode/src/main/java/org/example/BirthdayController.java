...
/*
Класс работает с датами дней рождения:
- хранит даты вчерашних, сегодняшних и завтрашних дней рождения у пользователей из списка контактов
- раз в день обновляет список дней рождения
- хранит состояние даты дня рождения - её можно скрывать

Нужен для работы классов
ProfileBirthdayEffect, анимирующего профиль пользователя, у которого день рождения (субмодуль ui/),
ProfileGiftsContainer, показывающего список подарков пользователя (субмодуль ui/Components/Premium/boosts),
StarsIntroActivity, работающего со "звёздами" пользователя (cубмодуль ui/stars/)
и UserSelectorBottomSheet (субмодуль ui/Components/Premium/boosts).
 */
